...
export default function App() {
  ...
  const [allPeriodRecords, setAllPeriodRecords] = useState<PeriodRecord[]>([]);
        ...
        setTodayDaily(null);
          ...
          useEffect(() => {
            ...
            const loadAllPersonalData = async () => {
              // 並列でデータを取得
              const [smiResult, dailyRecords, latestPeriodResult, allPeriods] = await Promise.all([
                storage.loadSMIResult(),
                storage.loadAllDailyRecords(),
                storage.getLatestPeriod(),
                storage.loadAllPeriods(),
              ]);
              ...
              setAllPeriodRecords(allPeriods);
            };
          ...
          const handleStartDailyCheck = () => {
            ...
            nav.navigate("daily");
                      ...
                      const handleSaveDaily = async (data: DailyRecord) => {
                        ...
                        // 保存後に全履歴と最新生理日を再取得してカレンダーとカード表示を更新
                        // saveDailyRecord が periods テーブルを更新するようになったため、この再取得で最新情報が手に入る
                        const [updatedRecords, updatedLatestPeriod, updatedAllPeriods] = await Promise.all([
                          storage.loadAllDailyRecords(),
                          storage.getLatestPeriod(),
                          storage.loadAllPeriods(),
                        ]);
                        setHistoryRecords(updatedRecords);
                        setLatestPeriod(updatedLatestPeriod);
